...
export function isReleaseSemVer(tag: string): boolean {
	const releaseSemVerRegExp = new RegExp(`^${releaseSemVer}$`);
...
export function isPrereleaseSemVer(tag: string): boolean {
	const prereleaseSemVerRegExp = new RegExp(
		`^${releaseSemVer}-${preReleaseSegment}(?:\\.${preReleaseSegment})*$`,
	);
	return prereleaseSemVerRegExp.test(tag);
}
